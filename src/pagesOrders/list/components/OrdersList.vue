  ...
  <scroll-view v-else-if="isShow" scroll-y class="orders">
    <view class="card" v-for="item in ordersList" :key="item.id">
      ...
      <navigator
        ...
        :key="sku.id"
        class="goods"
        :url="`/pagesOrders/detail/detail?id=${sku.spuId}`"
        hover-class="none"
      >
        <view class="cover">
      ...
      <view class="action">
        ...
        <template v-if="item.orderState === OrderState.DaiFuKuan">
          <view class="button primary" @click="toPayOrders(item.id)">去支付</view>
        </template>
        <template v-else>
          <navigator
            class="button secondary"
            :url="`/pagesOrders/create/create?orderId=${item.id}`"
            hover-class="none"
          >
            再次购买
          </navigator>
          <!-- 待收货状态: 展示确认收货 -->
          <view v-if="item.orderState === OrderState.DaiFaHuo" class="button primary"
            >确认收货</view
          >
        </template>
      </view>
...
import { getOrdersListApi, payOrdersApi, payOrdersMockApi } from '@/services/apis/orders'
...
const toPayOrders = async (orderId: string) => {
  ...
  const orders = ordersList.value!.find((v) => v.id === orderId)
  orders!.orderState = OrderState.DaiPingJia
}
onMounted(() => {
  getOrdersList()
})
